...
class CorrelationsTests(TestCase):
    ...
    def test_correlate_metadata(self):
        ...
        with self.assertRaisesRegex(ValueError, 'Not all values in column "age" can be interpreted as floats!'):
            correlate_metadata(self.metadata, categorials=[], ordinals={}, intervals=['age'])
        ...
        with self.assertRaisesRegex(ValueError, 'Mapping for ordinal "age" must be either None or a list but not a "<class \'str\'>"!'):
            correlate_metadata(self.metadata, categorials=[], ordinals={'age': 'AD'}, intervals=[])

        with self.assertRaisesRegex(ValueError, "You need to specify at least two columns!"):
        ...
        with self.assertRaisesRegex(ValueError, "time data '05\/23\/1999' does not match format '\%d\/\%Y'"):
            correlate_metadata(self.metadata, categorials=[], ordinals={}, dates={'collection_timestamp': "%d/%Y"}, intervals=[])
        ...
        x = correlate_metadata(
            ...
            err=e)
        self.assertEqual(e.getvalue(), 'Ordinal "age" does not specify label(s) "HY", "U".\n')

        # should be OK
        # correlate_metadata(self.metadata, categorials=['bird_family'], ordinals={}, intervals=[])
        # correlate_metadata(self.metadata, categorials=['bird_family'], ordinals={'age': None}, intervals=[])
        # correlate_metadata(self.metadata, categorials=[], ordinals={}, intervals=['latitude'])
